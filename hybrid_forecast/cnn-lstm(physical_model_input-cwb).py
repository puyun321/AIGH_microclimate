# -*- coding: utf-8 -*-
"""
Created on Tue Sep 21 14:40:55 2021
...
from keras.layers import Convolution1D,Dense,concatenate,LSTM
...
# Move back three levels
os.chdir(os.path.join(os.getcwd(), r'..\..\..'))
target_path = os.path.join(os.getcwd(), r'direct_forecast\indoor(CWB)\model_program')
os.chdir(target_path)

#physical model output (indoor temp)
physical_model_temp=pd.read_excel("model_performance/physical_model-performance.xlsx",sheet_name='temp_forecast')
...
training_input_four=np.expand_dims(physical_model_temp.iloc[train_index,2:],axis=2)
...
testing_input_four=np.expand_dims(physical_model_temp.iloc[test_index,2:],axis=2)
...
#outdoor ioT
# inputs2 = Input(shape=(6,training_input_two.shape[2]))
# output2=Convolution1D(filters=6,kernel_size=3,padding='same')(inputs2)
# output2=BatchNormalization()(output2)
# output2=Activation('relu')(output2)
# output2=Convolution1D(filters=12,kernel_size=3,padding='same')(output2)
# output2=BatchNormalization()(output2)
# output2=Activation('relu')(output2)
...
#operation
# inputs3 =Input(shape=(6,training_input_three.shape[2]))
...
inputs4 =Input(shape=(timestep,training_input_four.shape[2]))
output4=Convolution1D(filters=6,kernel_size=3,padding='same')(inputs4)
output4=BatchNormalization()(output4)
output4=Activation('relu')(output4)
output4=Convolution1D(filters=12,kernel_size=3,padding='same')(output4)
output4=BatchNormalization()(output4)
output4=Activation('relu')(output4)
# output4=LSTM(36,kernel_regularizer=regularizers.l2(0.01),stateful=False,return_sequences=True)(inputs4)
# output4=LSTM(36,kernel_regularizer=regularizers.l2(0.01),stateful=False,return_sequences=True)(output4)
output4=Flatten()(output4)

merge_output=concatenate([output,output4],axis=-1)
...
model = Model(inputs=[inputs1,inputs4], outputs=final_output)
# model = Model(inputs=[inputs1,inputs2,inputs4], outputs=final_output)

learning_rate=1e-4
adam = Adam(learning_rate=learning_rate)
model.compile(optimizer=adam,loss="mse")
earlystopper = EarlyStopping(monitor='val_loss', patience=15, verbose=0)        
save_path="model/cnn-lstm(cwb).hdf5"
checkpoint =ModelCheckpoint(save_path,save_best_only=True)
callback_list=[earlystopper,checkpoint]        
train_history=model.fit([training_input_one,training_input_four], training_output, epochs=100, batch_size=32,validation_split=0.2,callbacks=callback_list,shuffle=True)

#%% plot training and validation loss figure
...
#%%  forecasting and denormalization
# hybrid model forecasting
model=load_model(save_path) 
pred_train_norm=model.predict([training_input_one,training_input_four],batch_size=32)
pred_test_norm=(model.predict([testing_input_one,testing_input_four],batch_size=32))
...
writer = pd.ExcelWriter('model_performance/cnn-lstm-performance(cwb).xlsx', engine='xlsxwriter')
