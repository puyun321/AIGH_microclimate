...
#%%
database_path='D:/lab/research/溫室/demo/dataset/伸港Gh/2021 dataset'
...
# indoor_model_output=model_output(norm_indoor_output,timestep,same_indoor_index,8,error_index) #forecast 8 features
# outdoor_model_output=model_output(norm_outdoor_output,timestep,same_indoor_index,5,error_index) #forecast 5 features
...
selected_outdoor_output=np.concatenate([outdoor_model_output[:,np.asarray(outdoor_factor[selected_output_factor,i])] for i in range(0,timestep+1)],axis=1)
denorm_use=np.concatenate([outdoor_output.iloc[:,np.asarray(outdoor_factor[selected_output_factor,i])] for i in range(0,timestep+1)],axis=1) #outdoor output
...
pred_factor=find_feature_num(training_output,3) #indoor
#%%
...
pred_factor=find_feature_num(training_output,feature_num) #indoor
factor_=np.asarray(pred_factor)
writer = pd.ExcelWriter('model_performance/cnn-lstm-performance(so).xlsx', engine='xlsxwriter')
for i in range(0,feature_num):
    
    sheet_name=indoor.columns[1:]
    sheet_name=sheet_name[selected_output_factor[i]]
    ...
    final_output=Dense(len(training_output[0,factor_[i,:]]),kernel_regularizer=regularizers.l2(0.01))(final_output)
    ...
    save_path="model/cnn-lstm(%s).hdf5"%sheet_name
    checkpoint =ModelCheckpoint(save_path,save_best_only=True)
    callback_list=[earlystopper,checkpoint]        
    model.fit(training_input_one, training_output[:,factor_[i,:]], epochs=100, batch_size=32,validation_split=0.2,callbacks=callback_list,shuffle=True)
    ...
    pred_train=denormalize(denorm_use[:,factor_[i,:]],pred_train_norm)
    pred_test_norm=(model.predict(testing_input_one,batch_size=32))
    pred_test=denormalize(denorm_use[:,factor_[i,:]],pred_test_norm)
    
    training_output_denorm=denormalize(denorm_use[:,factor_[i,:]],training_output[:,factor_[i,:]])
    testing_output_denorm=denormalize(denorm_use[:,factor_[i,:]],testing_output[:,factor_[i,:]])

#calculate R2 and save into excel

    for k in range(0,19):          

        training_R2[i].append(error_indicator.np_R2(training_output_denorm[:,k],pred_train[:,k]))
        testing_R2[i].append(error_indicator.np_R2(testing_output_denorm[:,k],pred_test[:,k]))
        training_RMSE[i].append(error_indicator.np_RMSE(training_output_denorm[:,k],pred_train[:,k]))
        testing_RMSE[i].append(error_indicator.np_RMSE(testing_output_denorm[:,k],pred_test[:,k]))
        training_mape[i].append(error_indicator.np_mape(training_output_denorm[:,k],pred_train[:,k]))
        testing_mape[i].append(error_indicator.np_mape(testing_output_denorm[:,k],pred_test[:,k]))   
        training_mae[i].append(error_indicator.np_mae(training_output_denorm[:,k],pred_train[:,k]))
        testing_mae[i].append(error_indicator.np_mae(testing_output_denorm[:,k],pred_test[:,k]))   
    ...
    final_forecast_result.to_excel(writer,sheet_name="%s_forecast"%sheet_name)
    model_real_output=pd.concat([pd.DataFrame(training_output_denorm),pd.DataFrame(testing_output_denorm)]).reset_index(drop=True)
    final_real_output=indoor_info
    final_real_output=pd.concat([final_real_output,model_real_output],axis=1)
    final_real_output.to_excel(writer,sheet_name="%s_realoutput"%sheet_name)
