# -*- coding: utf-8 -*-
"""
Created on Mon Sep 20 17:08:02 2021
...
#extract date info
indoor_info=indoor.iloc[np.squeeze(np.asarray(same_indoor_index.iloc[1:,0])),0]
...
denorm_use=np.concatenate([indoor_output.iloc[:,np.asarray(indoor_factor[selected_output_factor,i])] for i in range(0,7)],axis=1) #outdoor output
...
save_path="model/lstm-lstm.hdf5"
checkpoint =ModelCheckpoint(save_path,save_best_only=True)
callback_list=[earlystopper,checkpoint]        
model.fit([training_input_one,training_input_two,training_input_three], training_output, epochs=100, batch_size=32,validation_split=0.2,callbacks=callback_list,shuffle=True)

#%% hybrid model forecasting
# forecasting
model=load_model(save_path) 
pred_train_norm=model.predict([training_input_one,training_input_two,training_input_three],batch_size=32)
pred_test_norm=(model.predict([testing_input_one,testing_input_two,testing_input_three],batch_size=32))
...
writer = pd.ExcelWriter('model_performance/lstm-performance.xlsx', engine='xlsxwriter')
