...
#%%
#same index cwb, outdoor, indoor 
same_element_=reduce(np.intersect1d, (operation_datetime,outdoor_datetime,indoor_datetime))
same_index_operation_=np.squeeze(np.asarray([np.argwhere(operation_datetime==same_element_[i]) for i in range(0,len(same_element_))]))
same_index_outdoor_=np.squeeze(np.asarray([np.argwhere(outdoor_datetime==same_element_[i]) for i in range(0,len(same_element_))]))
same_index_indoor_=np.squeeze(np.asarray([np.argwhere(indoor_datetime==same_element_[i]) for i in range(0,len(same_element_))]))
...
#%%
writer = pd.ExcelWriter('D:/database/溫室/伸港Gh/same_index(10min).xlsx', engine='xlsxwriter')
...
writer.save()
cwb_combine.to_csv("D:/database/溫室/伸港Gh/CWB_data/cwb_combine(clean)-10min.csv")

indoor_output.to_csv("D:/database/溫室/伸港Gh/indoor_10min_output(clean).csv")
outdoor_output.to_csv("D:/database/溫室/伸港Gh/outdoor_10min_output(clean).csv")
index_2_remove.to_csv("D:/database/溫室/伸港Gh/extra_remove_index(10min).csv")

#%%
writer = pd.ExcelWriter('D:/database/溫室/伸港Gh/same_index(10min)_.xlsx', engine='xlsxwriter')
#save same index for cwb,outdoor,indoor  
pd.DataFrame(same_index_operation_).to_excel(writer,sheet_name="operation_index")
pd.DataFrame(same_index_outdoor_).to_excel(writer,sheet_name="outdoor_index")
pd.DataFrame(same_index_indoor_).to_excel(writer,sheet_name="indoor_index")
writer.save()
